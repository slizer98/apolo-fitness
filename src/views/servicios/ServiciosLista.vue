<template>
  <div class="p-4 text-white">
    ...
    <div class="mb-4 flex flex-wrap gap-2">
      <input v-model="q" @keyup.enter="fetch" placeholder="Buscar servicio…"
    ...
    <table v-else class="w-full text-sm">
      <thead class="text-gray-400">
        <tr>
          ...
          <th class="text-left pb-2">Icono</th>
          <th class="text-right pb-2">Acciones</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="s in rows" :key="s.id" class="border-t border-gray-800/80 hover:bg-gray-900/40">
          <td class="py-2">{{ s.nombre }}</td>
          <td class="py-2 text-gray-300">{{ s.descripcion || '—' }}</td>
          <td class="py-2 text-gray-300">{{ s.icono || '—' }}</td>
          <td class="py-2">
            <div class="flex items-center justify-end gap-2">
              <button @click="openEdit(s)" class="px-2 py-1 rounded border border-gray-700 bg-gray-800/60 hover:bg-gray-700">Editar</button>
              <button @click="confirmRemove(s)" class="px-2 py-1 rounded border border-red-800 bg-red-900/40 hover:bg-red-800">Eliminar</button>
            </div>
          </td>
        </tr>
        <tr v-if="!rows.length">
        ...
        <form @submit.prevent="save" class="p-4 space-y-4">
          <div class="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input v-model="form.icono" placeholder="fa-dumbbell" class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2" />
            </div>
          </div>
...
async function fetch() {
  loading.value = true
  try {
    const { data } = await api.servicios.list({ search: q.value, page: page.value, page_size: pageSize, ordering: '-id' })
...
function openNew(){
  ...
  errors.value = {}
  form.value = { id:null, nombre:'', descripcion:'', icono:'' }
  showModal.value = true
}
function openEdit(s){
  isEditing.value = true
  errors.value = {}
  form.value = { id: s.id, nombre: s.nombre || '', descripcion: s.descripcion || '', icono: s.icono || '' }
  showModal.value = true
}
