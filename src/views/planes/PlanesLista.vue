<template>
  <div class="p-4 text-white">
    <div class="flex items-center justify-between mb-4">
      ...
      <RouterLink :to="{ name: 'PlanCrear' }"
        class="bg-apolo-primary text-black px-4 py-2 rounded hover:bg-apolo-secondary transition">+ Nuevo</RouterLink>
    </div>

    <!-- Filtros -->
    <div class="mb-4 flex flex-wrap gap-2">
      <input v-model="q" @keyup.enter="fetch" placeholder="Buscar plan…"
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <article v-for="p in rows" :key="p.id"
        ...
        <div class="mt-2 flex flex-wrap gap-2 text-[11px]">
          ...
          <span v-if="p.desde || p.hasta" class="px-2 py-0.5 rounded-full bg-gray-800/60 border border-gray-700">
            Vigencia: {{ formatRange(p.desde, p.hasta) }}
          </span>
        </div>

        <div class="mt-3 flex items-center justify-end gap-2">
          <RouterLink :to="{ name:'PlanDetalle', params:{ id: p.id }}"
            class="px-2 py-1 rounded border border-gray-700 bg-gray-800/60 hover:bg-gray-700">Ver</RouterLink>
          <RouterLink :to="{ name:'PlanEditar', params:{ id: p.id }}"
            class="px-2 py-1 rounded border border-gray-700 bg-gray-800/60 hover:bg-gray-700">Editar</RouterLink>
          <button @click="remove(p)" class="px-2 py-1 rounded border border-red-800 bg-red-900/40 hover:bg-red-800">Eliminar</button>
        </div>
      </article>
